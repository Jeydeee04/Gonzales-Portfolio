        ...
        <header className="fixed inset-x-0 top-0 z-40 p-3 md:inset-x-auto md:h-screen md:p-3">
            <nav className="relative flex h-auto w-full flex-row items-center justify-between gap-4 rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm md:h-full md:w-auto md:flex-col md:justify-center md:gap-15 md:rounded-2xl md:rounded-r-2xl md:border-r-2 md:border-t-0 md:border-b-0 md:border-l-0 md:px-14 md:pb-40 md:shadow-none">
                ...
                <ul className="hidden items-center gap-5 text-gray-700 md:flex md:flex-col">
                    {navItems.map((item, i) => (
                        <li key={i}>
                            <a href={item.href} className="group relative inline-flex items-center gap-2 py-1 text-base font-medium transition-all duration-300 ease-out hover:-translate-y-1 hover:scale-110 hover:text-black">
                                <span className="text-gray-600 opacity-0 transition-all duration-300 ease-out -translate-x-2 group-hover:translate-x-0 group-hover:opacity-100">&lt;</span>
                                <span>{item.name}</span>
                                <span className="text-gray-600 opacity-0 transition-all duration-300 ease-out translate-x-2 group-hover:translate-x-0 group-hover:opacity-100">&gt;</span>
                    ...
                    {isMenuOpen && (
                        ...
                        >
                            {navItems.map((item, i) => (
                                ...
                                >
                                    <a href={item.href} onClick={() => setIsMenuOpen(false)} className="group relative inline-flex w-full items-center rounded-xl px-3 py-2 text-sm font-medium transition-colors hover:bg-gray-100 hover:text-black sm:text-base">
                                        <span>{item.name}</span>
