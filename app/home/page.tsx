...
import { FiUser, FiFileText, FiMail, FiArrowRight } from "react-icons/fi";
import { FaLinkedinIn, FaGithub, FaFacebookF } from "react-icons/fa";
import { SiCplusplus, SiC, SiPython, SiPhp, SiReact, SiNextdotjs, SiTailwindcss, SiDocker, SiMongodb, SiMysql, SiFirebase, SiGit, SiPytorch, SiTensorflow } from "react-icons/si";
...
const projects = [
  {
    ...
    description: "An AI-powered garbage bin that uses image classification to automatically classify and segregate waste in real-time.",
    tags: ["Python", "React Native", "YOLOv8", "C++"],
  },
  {
    title: "PPE Management System",
    description: "A web-based inventory management system for the Department of Agrarian Reform — tracks assets across fund clusters with dashboard analytics and CSV export.",
    tags: ["PHP", "MySQL", "Tailwind CSS", "Docker"],
  },
];
  ...
  return (
    <>
      ...
      <section className="py-20 noise-overlay">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center gap-4 mb-10 animate-fade-in-up">
            <h2 className="font-bebas text-4xl text-white uppercase tracking-wider text-hierarchy-high">Projects</h2>
            <span className="flex-1 h-[1px] bg-red-600/20" />
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            {projects.map((project, i) => (
              <div key={project.title} className="glass-card p-6 animate-fade-in-up" style={{ animationDelay: `${(i + 1) * 150}ms`, clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))" }}>
                <h3 className="font-bebas text-xl text-white uppercase tracking-wider text-hierarchy-high">{project.title}</h3>
                <p className="mt-2 text-sm text-gray-400 leading-relaxed text-hierarchy-medium">{project.description}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  {project.tags.map((tag) => (
                    <span key={tag} className="px-2 py-1 text-[10px] font-black text-gray-400 border border-white/10 rounded-full uppercase tracking-[0.15em] text-hierarchy-low">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="mt-8 animate-fade-in-up delay-300">
            <Link
              href="/projects"
              className="group inline-flex items-center gap-2 text-sm font-bold text-gray-400 hover:text-red-600 transition-colors uppercase tracking-[0.2em]"
            >
              View All Projects
      ...
      <section className="py-20 noise-overlay">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center gap-4 mb-10 animate-fade-in-up">
            <h2 className="font-bebas text-4xl text-white uppercase tracking-wider text-hierarchy-high">Tech Stack</h2>
            <span className="flex-1 h-[1px] bg-red-600/20" />
          </div>
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-6 animate-fade-in-up delay-100">
            {techStack.map((tech) => (
              <div key={tech.name} className="flex flex-col items-center gap-2 group">
                <span className="text-2xl text-gray-500 group-hover:text-red-600 transition-colors duration-200">{tech.icon}</span>
                <span className="text-[10px] font-bold text-gray-500 group-hover:text-white uppercase tracking-wider transition-colors duration-200 text-hierarchy-low">{tech.name}</span>
              </div>
            ))}
          </div>
